...
def create_additional_plots(title, name_data_paths, set_number, output_name):
    ...
    try:
        ...
        if len(name_data_paths) > 50:
            debug_center.show_in_console(None, "Warning", ("WARNING: Too many plots given, merged plot would not make any sense."))
            return -1
        ...
        good_width = max(1200, int(1200 / 80 * len(datasets[0])))
        ...
        plt_file.write("\n")
        plt_file.write("plot " + ",".join(["data_file index {0} using 2:xtic(1) title \"{1}\" with points pointtype 7 pointsize 2".format(i,name) for (i,name) in enumerate(names)]))
        plt_file.write("\n")
        plt_file.close()
