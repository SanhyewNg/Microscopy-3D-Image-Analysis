...
def determine_output_filepath(filepath, output_path):
    if os.path.isabs(filepath) == True:
        ...
        # Case for git-lfs, where paths to files are relative (with regard
        # to repository directory tree, e.g. data/evaluator/results). We want
        # it to work across various machines, without sticking to any absolute
        # paths.
        if os.path.isabs(output_path) == True:
            raise NotImplementedError
        else:
            # Behave in a similar way as for absolute filepath.
            # Let output be one level up.
            components = split_path(filepath)
            return os.path.join(os.path.join(*components[:-1]), output_path, 
                                             os.path.join(*components[-1:]))
