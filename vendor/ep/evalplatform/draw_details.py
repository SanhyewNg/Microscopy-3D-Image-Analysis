...
# To prevent `tkinter.TclError: no display name and no $DISPLAY environment
# variable` error.
import matplotlib
matplotlib.use('Agg')
...
class PaintRequestABC(object):
    ...
    def draw_cell(self,axis,position,result):
        ...
        axis.plot(position[0],position[1],"o", alpha=1, markersize=markersize, markerfacecolor = colours[result], markeredgecolor=colours[result])
...
def run(overlord, directory_images, directory_output, desired_output_file_prefix = None):
    ...
    for (frame, data_piece) in data:
        frame_num = get_trailing_number(frame)
        if frame_num in image_number_dict:
            image = image_number_dict[frame_num]
            requests = requests + overlord.create_paint_request(frame_num, image, data_piece)
